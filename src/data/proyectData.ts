...
export const projectsData: ProjectProps[] = [
  ...
  },
  {
    ...
    title: "Atiende — Autonomous WhatsApp Business AI Agent",
    description:
      "Atiende is an autonomous conversational agent for WhatsApp Business built as a provider-agnostic harness: it orchestrates messages through a resilient pipeline (verified webhook → zero-loss persistence → BullMQ queue → agent tool loop) and speaks to any LLM through a common port, with circuit breaker, fallback routing, multi-level caching, and human escalation.",
    shortDescription: "Provider-agnostic AI harness for WhatsApp conversations",
    category: "featured",
    status: "in-progress",
    companyName: "Latinoamerican SMBs (multi-tenant) — retail and services sectors",
    tech: [
      "NestJS",
      "TypeScript",
      "BullMQ",
    ...
    techTags: {
      ...
      "Queue & Real-time": ["BullMQ", "Redis", "Meta Cloud API Webhooks"],
      "Database": ["PostgreSQL", "pgvector (semantic cache)"],
      "AI Providers": ["Groq (primary)", "OpenAI", "Gemini", "Kimi K3", "Claude (pending)"],
    ...
    features: [
      ...
      "BullMQ queue with exponential backoff — the webhook never waits for the LLM",
      "Agent tool loop (RAG catalog, orders, escalation, knowledge) with iteration limit",
      "LLM provider switching via feature flags — no code changes",
      "Circuit breaker + automatic fallback routing (primary → CB → fallback)",
      "Multi-level response caching: exact (Redis) + semantic (pgvector)",
      "Scope guard and hallucination validation to keep the bot safe",
      "Per-conversation budget and strict timeouts/abort control",
      "Human escalation with dashboard, plus per-turn telemetry (tokens, USD cost, latency)",
    ],
    repositoryVisibility: "private",
    role: "Solution Architect and Lead Developer (sole developer)",
    achievements: [
      "Architected a hexagonal, provider-agnostic harness as sole developer",
      "Webhook with < 200 ms ack and zero message loss",
      "6 LLM adapters (Groq, OpenAI, Gemini, Kimi K3, Mock; Claude pending) without touching the core",
      "Fallback wiring with circuit breaker shipped",
      "Conversation cost < $0.05 via exact + semantic caching",
      "Multi-tenant: one instance, N businesses, each with its own number and prompt",
    ],
    caseStudy: {
      problem: "Latinoamerican SMBs lose sales because they don't answer WhatsApp in time, and fixed-response chatbots can't handle real conversations. On top of that, coupling the system to a single AI provider creates vendor lock-in: switching models (or the provider going down) meant rewriting the core.",
      solution: "I designed Atiende as a harness: an orchestration layer between the channel and the model. A Meta webhook with verified HMAC signature answers 200 in under 200 ms and persists the message before enqueueing it (zero loss). BullMQ processes the turn without blocking the webhook: the AgentService builds the prompt, calls the LLM, executes tools (RAG catalog, orders, escalation) in a loop with an iteration limit, and validates the response. Every LLM enters through a single contract (LLMProviderPort) with portable hints (cacheable, effort, forceTool, signal); a circuit breaker + router decides primary → fallback, and a multi-level cache (exact Redis + semantic pgvector) serves traffic without calling the model.",
      results: [
        "p95 < 5 s per turn and < $0.05 per average conversation thanks to multi-level caching",
        "Zero message loss: persist-before-enqueue + at-least-once + idempotency",
        "6 LLM adapters implemented (Groq, OpenAI, Gemini, Kimi K3, Mock; Claude pending) without touching the core",
        "Provider fallback with circuit breaker wired (primary → CB → fallback)",
        "Designed for 100 businesses / 10k messages per day per instance (multi-tenant)",
        "Human escalation with dashboard, per-conversation budget, and scope guard",
      ],
      keyLearnings: [
        ...
        "Multi-level caching (exact + semantic) is what makes cost viable, not the model itself",
        "Reasoning models (Kimi K3) require propagating reasoning_content + tool_calls through the loop",
      ],
      challenges: [
        ...
        "Keeping the bot safe: scope guard classifier, hallucination validator, per-conversation budget, and strict iteration limits/timeouts",
      ],
    },
  },
  // {
